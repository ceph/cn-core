...
VERSION="$1"
TARBALL="cn-core-vendor-$VERSION.tar.xz"
...
CURRENT_DIR=$(pwd)
TMPDIR=$(mktemp -d /tmp/cn-core-vendor.XXXX)
pushd "$TMPDIR" &>/dev/null
  mkdir src
  pushd src &>/dev/null
    wget -q https://github.com/ceph/cn-core/archive/v"$VERSION".tar.gz
    tar -xf "v$VERSION.tar.gz"
    pushd "cn-core-$VERSION" &>/dev/null
